...
When("cierra la sesión", async ({ page }) => {
  ...
  // La portada del entorno que se este probando, no una URL fija.
  const portada = new URL(process.env.E2E_BASE_URL ?? "http://localhost:3001");
  await expect(page).toHaveURL(
    new RegExp(`${portada.host.replace(/\./g, "\\.")}\\/$`),
    { timeout: 20_000 },
  );
});
